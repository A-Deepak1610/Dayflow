  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500">Review, approve, or reject employee leave requests with HR notes</p>
      </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 shrink-0">
                {req.status === 'Pending' ? (
                    ...
                    {activeCommentId === req.id ? (
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-700 text-white rounded-xl text-xs font-bold shadow-xs cursor-pointer"
                        >
                          Reject
                        </button>
                      </div>
                  ...
                  <span className={`text-xs font-bold px-3.5 py-1.5 rounded-full ${
                    req.status === 'Approved' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-slate-200 text-slate-700 border border-slate-300'
                  }`}>
                    {req.status}
                  </span>
                )}
